...
public class HomeFragment extends Fragment implements SearchView.OnQueryTextListener, IFetchListData {
    ...
    private String homeUrl = "https://id.nhent.ai?page=";
    private int page = 1;
    ...
    public void setTag(String tagUrl, int option) {
        ...
        switch (option) {
            case HOME_OPTION:
                homeUrl = "https://id.nhent.ai/?page=";
                setHomePage(true);
                break;
            case GENRE_OPTION:
                homeUrl = "https://id.nhent.ai" + tagUrl + "?page=";
                setHomePage(false);
                break;
            case TITLE_OPTION:
                homeUrl = "https://id.nhent.ai/search/?q=" + tagUrl + "&page=";
                setHomePage(false);
                break;
        }
    ...
    public List<Manga> parseResponse(String html) {
        ...
        for (Element el : gallery) {
            ...
            String coverImage = el.getElementsByTag("img").attr("data-src").replace("//t.nhentai.net", "//kontol.nhent.ai");;
